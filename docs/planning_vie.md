...
| T3 | 10/03 | **BE2** | Thiết kế schema CSDL: định nghĩa tất cả các thực thể chính và quan hệ cho ứng dụng | Sơ đồ ERD hoặc file migration được review và commit |
| T3 | 10/03 | **BE1** | Thiết lập SQLAlchemy models + Alembic migration cho tất cả thực thể chính | Migration chạy thành công; các bảng được tạo trong PostgreSQL |
...
| T4 | 11/03 | **BE1** | Triển khai API xác thực cho chức năng đăng ký, đăng nhập và làm mới token với JWT + bcrypt | API xác thực trả JWT token chính xác; mật khẩu được lưu dạng bcrypt hash |
...
| T5 | 12/03 | **BE2** | Middleware RBAC (vai trò Admin/User), decorator phân quyền | Tài nguyên được bảo vệ trả về `403` cho vai trò không được phép |
| T5 | 12/03 | **BE1** | Hoàn thiện xác thực: triển khai chức năng đăng xuất, đổi mật khẩu và xác minh email | Tất cả tính năng xác thực được test và ghi nhận trên Swagger |
...
| T6 | 13/03 | **BE2** | Tích hợp Redis: middleware giới hạn tần suất (cấu hình được số requests/phút/user) | Request bị giới hạn trả `429` khi vượt ngưỡng; giới hạn có thể cấu hình |
| T6 | 13/03 | **BE1** | Triển khai API quản lý người dùng cho chức năng lấy/cập nhật hồ sơ và cài đặt tài khoản | API quản lý người dùng trả về và cập nhật dữ liệu user chính xác |
...
| T3 | 17/03 | **BE1** | Triển khai API dữ liệu giá cho chức năng truy xuất báo giá thời gian thực, dữ liệu lịch sử và truy vấn hàng loạt | API trả dữ liệu giá có cấu trúc với khoảng thời gian chính xác |
...
| T4 | 18/03 | **BE1** | Triển khai API chỉ báo kỹ thuật: tính toán SMA, EMA, RSI, MACD | API trả giá trị tính toán chính xác cho symbol/khung thời gian cho trước |
...
| T3 | 24/03 | **BE2** | Triển khai MCP tools: công cụ lấy dữ liệu giá cổ phiếu và phân tích chỉ báo kỹ thuật | Tools gọi được qua giao thức MCP; trả dữ liệu live chính xác |
...
| T4 | 25/03 | **BE2** | Triển khai MCP tools: công cụ truy xuất sự kiện lịch kinh tế và tóm tắt tin tức | Tất cả MCP tools trả dữ liệu chính xác, đã format |
...
| T5 | 26/03 | **BE2** | Triển khai streaming dựa trên SSE cho phản hồi AI theo từng từ | SSE stream gửi token khi được sinh ra; client nhận text tăng dần |
...
| T6 | 27/03 | **BE1** | Triển khai API chat cho chức năng gửi tin nhắn, truy xuất lịch sử chat và liệt kê hội thoại | API chat hoạt động end-to-end: gửi tin → nhận phản hồi AI stream |
...
| T5 | 02/04 | **BE2** | Triển khai API tin tức cho chức năng liệt kê, lọc (theo ngày/cảm xúc/nguồn) và truy xuất bài đơn lẻ | API trả tin tức phân trang với bộ lọc; truy xuất bài đơn lẻ hoạt động |
...
| T6 | 03/04 | **BE2** | Triển khai API lịch kinh tế cho chức năng liệt kê và lọc sự kiện theo ngày, quốc gia và mức quan trọng | API lịch trả sự kiện có cấu trúc với bộ lọc chính xác |
...
| T3 | 07/04 | **BE2** | Triển khai API CRUD Portfolio cho chức năng thêm, xóa và sửa holdings với số lượng, giá mua và ngày | Tất cả CRUD hoạt động; giá trị portfolio tính từ giá live |
...
| T4 | 08/04 | **BE2** | Triển khai API CRUD Watchlist cho chức năng thêm/xóa symbol, sắp xếp lại và cài đặt tùy chọn | API Watchlist đã test; symbol lưu trữ theo tài khoản user |
...
| T3 | 14/04 | **BE2** | Triển khai API CRUD cảnh báo cho chức năng tạo, đọc, cập nhật, xóa và liệt kê cảnh báo đang hoạt động theo user | Tất cả API CRUD cảnh báo hoạt động; user quản lý được cảnh báo của mình |
...
| T5 | 16/04 | **BE2** | Triển khai API CRUD bài viết/post hỗ trợ nội dung markdown/HTML, thông tin tác giả và thời gian | Bài viết tạo, liệt kê, cập nhật, xóa qua API; nội dung hiển thị đúng |
...
| T6 | 17/04 | **BE2** | Triển khai API bình luận và đánh giá cho chức năng thêm/liệt kê bình luận và gửi/cập nhật đánh giá kèm tính trung bình | Thread bình luận hoạt động; đánh giá trung bình cập nhật đúng khi có submission mới |
